...
- **命令 / 步骤**: 前置检查均使用 5 秒显式超时执行；`.env` 检查返回 `ENV_MISSING`，凭据检查返回 `SKIP_CREDENTIAL_CHECK_ENV_MISSING`
- **错误摘要**: 本地模板、输出 schema、upstream learning 参考资料可用，但运行必需环境文件缺失，导致采集凭据状态不可验证
- **相关文件**: `.env`（缺失）、`templates/failure_alert.md`、`templates/daily_market_report.md`、`references/output-schema.md`、`references/upstream-learning.md`
